Vue.createApp({
  ...
  computed: {
    filterdColumnNames() {
      ...
      const columnResult = Object.keys(singleBookItem).filter((propKey) => {
        return validKeys.includes(propKey); // FILTER gibt  eine Kopie vom books-Object mit keys züruck, die in validKeys gespeichhert sind????????
      });
  ...
  methods: {
    ...
    // },
    dicount() {
      const priceString = this.books.price;
      console.log(priceString);

      const priceToNumber = Number(priceString.replace(/[^0-9.-]+/g, ""));
      console.log(priceToNumber);

      const newPrice = priceToNumber - priceToNumber * 0.2;
      return newPrice;
    },
  },
}).mount("#app");
